...
if sys.version_info < (2, 7):
    print(
        "Python versions prior to 2.7 are not supported for pip installed PySpark.",
        file=sys.stderr,
    )
    sys.exit(-1)
...
in_spark = os.path.isfile(
    "../core/src/main/scala/org/apache/spark/SparkContext.scala"
) or (os.path.isfile("../RELEASE") and len(glob.glob("../jars/spark*core*.jar")) == 1)
...
try:
    ...
    try:
        ...
        print(
            "Could not import pypandoc - required to package PySpark", file=sys.stderr
        )
    except OSError:
