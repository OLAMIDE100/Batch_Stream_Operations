...
class ExternalMerger(Merger):
    ...
    def _spill(self):
        ...
        if not self.pdata:
            ...
            streams = [
                open(os.path.join(path, str(i)), "wb") for i in range(self.partitions)
            ]
...
class ExternalGroupBy(ExternalMerger):
            ...
            for i in range(self.partitions):
                ...
                with open(p, "wb") as f:
                    ...
                    if self._sorted:
                        ...
                        sorted_items = sorted(
                            self.pdata[i].items(), key=operator.itemgetter(0)
                        )
    ...
    def _merge_sorted_items(self, index):
        ...
        if self._sorted:
            ...
            sorted_items = sorter.sorted(
                itertools.chain(*disk_items), key=operator.itemgetter(0)
            )
