...
class SparkSession(object):
    ...
    def __init__(self, sparkContext, jsparkSession=None):
        ...
        if jsparkSession is None:
            if (
                ...
                and not self._jvm.SparkSession.getDefaultSession()
                .get()
                .sparkContext()
                .isStopped()
            ):
    ...
    def range(self, start, end=None, step=1, numPartitions=None):
        ...
        if end is None:
            jdf = self._jsparkSession.range(
                0, int(start), int(step), int(numPartitions)
            )
        else:
            jdf = self._jsparkSession.range(
                int(start), int(end), int(step), int(numPartitions)
            )
    ...
    def _create_from_pandas_with_arrow(self, pdf, schema, timezone):
        ...
        from pyspark.sql.types import TimestampType, from_arrow_schema, to_arrow_type
        ...
        if isinstance(schema, StructType):
            ...
            raise ValueError(
                "Single data type %s is not supported with Arrow" % str(schema)
            )
        else:
        ...
        jrdd = self._sc._serialize_to_jvm(
            batches, ArrowStreamSerializer(), reader_func, create_RDD_server
        )
    ...
    def createDataFrame(self, data, schema=None, samplingRatio=None, verifySchema=True):
        ...
        if isinstance(schema, StructType):
            verify_func = (
                _make_type_verifier(schema) if verifySchema else lambda _: True
            )
