...
class DataFrameReader(OptionUtils):
    ...
    def load(self, path=None, format=None, schema=None, **options):
        ...
        if isinstance(path, basestring):
            ...
            if type(path) != list:
                path = [path]
            return self._df(
                self._jreader.load(self._spark._sc._jvm.PythonUtils.toSeq(path))
            )
        else:
    ...
    def json(
        ...
        if type(path) == list:
            return self._df(
                self._jreader.json(self._spark._sc._jvm.PythonUtils.toSeq(path))
            )
    ...
    def text(self, paths, wholetext=False, lineSep=None):
        ...
        if isinstance(paths, basestring):
            paths = [paths]
        return self._df(
            self._jreader.text(self._spark._sc._jvm.PythonUtils.toSeq(paths))
        )
    ...
    def csv(
        ...
        if type(path) == list:
            return self._df(
                self._jreader.csv(self._spark._sc._jvm.PythonUtils.toSeq(path))
            )
        elif isinstance(path, RDD):
    ...
    def jdbc(
        ...
        if properties is None:
            properties = dict()
        jprop = JavaClass(
            "java.util.Properties", self._spark._sc._gateway._gateway_client
        )()
        for k in properties:
        ...
        if column is not None:
            ...
            ), "lowerBound can not be None when ``column`` is specified"
            assert (
                upperBound is not None
            ), "upperBound can not be None when ``column`` is specified"
            assert (
...
class DataFrameWriter(OptionUtils):
    ...
    def bucketBy(self, numBuckets, col, *cols):
        ...
        if not all(isinstance(c, basestring) for c in cols) or not (
