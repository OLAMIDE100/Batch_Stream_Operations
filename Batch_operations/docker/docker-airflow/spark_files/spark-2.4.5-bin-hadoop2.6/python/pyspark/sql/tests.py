...
from pyspark.sql import Column, HiveContext, Row, SparkSession, SQLContext
from pyspark.sql.functions import UserDefinedFunction, input_file_name, lit, sha2
...
class SQLTests(ReusedSQLTestCase):
    ...
    def test_explode(self):
        ...
        self.assertEqual(
            result, [(0, "a", "b"), (None, None, None), (None, None, None)]
        )
    ...
    def test_and_in_expression(self):
        self.assertEqual(
            4, self.df.filter((self.df.key <= 10) & (self.df.value <= "2")).count()
        )
        self.assertRaises(
            ValueError, lambda: (self.df.key <= 10) and (self.df.value <= "2")
        )
        self.assertEqual(
            14, self.df.filter((self.df.key <= 3) | (self.df.value < "2")).count()
        )
    ...
    def test_udf(self):
        self.spark.catalog.registerFunction(
            "twoArgs", lambda x, y: len(x) + y, IntegerType()
        )
    ...
    def test_udf2(self):
        self.spark.catalog.registerFunction(
            "strlen", lambda string: len(string), IntegerType()
        )
    ...
    def test_nondeterministic_udf(self):
        ...
        udf_random_col = udf(
            lambda: int(100 * random.random()), IntegerType()
        ).asNondeterministic()
    ...
    def test_nondeterministic_udf2(self):
        ...
        random_udf = udf(
            lambda: random.randint(6, 6), IntegerType()
        ).asNondeterministic()
    ...
    def test_nondeterministic_udf_in_aggregate(self):
        ...
        udf_random_col = udf(
            lambda: int(100 * random.random()), "int"
        ).asNondeterministic()
    ...
    def test_multiple_udfs(self):
        ...
        [row] = self.spark.sql(
            "SELECT double(double(1)), double(double(2) + 2)"
        ).collect()
    ...
    def test_udf_not_supported_in_join_condition(self):
        ...
        def runWithJoinType(join_type, type_string):
            with self.assertRaisesRegexp(
                ...
                left.join(
                    right, [f("a", "b"), left.a1 == right.b1], join_type
                ).collect()
    ...
    def test_broadcast_in_udf(self):
        ...
        self.spark.catalog.registerFunction(
            "MYUDF", lambda x: foo.value[x] if x else ""
        )
    ...
    def test_udf_with_filter_function(self):
        df = self.spark.createDataFrame(
            [(1, "1"), (2, "2"), (1, "2"), (1, "2")], ["key", "value"]
        )
        from pyspark.sql.functions import col, udf
        from pyspark.sql.types import BooleanType

        my_filter = udf(lambda a: a < 2, BooleanType())
        sel = df.select(col("key"), col("value")).filter(
            (my_filter(col("key"))) & (df.value < "2")
        )
    ...
    def test_udf_in_generate(self):
        ...
        range_udf = udf(
            lambda value: list(range(value - 1, value + 1)), ArrayType(IntegerType())
        )
    ...
    def test_linesep_text(self):
        df = self.spark.read.text(
            "python/test_support/sql/ages_newlines.csv", lineSep=","
        )
        expected = [
            Row(value="Joe"),
            Row(value="20"),
    ...
    def test_encoding_json(self):
        people_array = self.spark.read.json(
            "python/test_support/sql/people_array_utf16le.json",
            multiLine=True,
            encoding="UTF-16LE",
        )
    ...
    def test_input_file_name_reset_for_rdd(self):
        rdd = self.sc.textFile("python/test_support/hello/hello.txt").map(
            lambda x: {"data": x}
        )
    ...
    def test_udf_with_string_return_type(self):
        ...
        make_pair = UserDefinedFunction(
            lambda x: (-x, x), "struct<x:integer,y:integer>"
        )
    ...
    def test_validate_column_types(self):
        ...
        self.assertTrue("Column" in _to_java_column("a").getClass().toString())
        self.assertTrue(
            "Column" in _to_java_column(self.spark.range(1).id).getClass().toString()
        )
    ...
    def test_apply_schema(self):
        ...
        schema = StructType(
            [
                ...
                StructField(
                    "struct1", StructType([StructField("b", ShortType(), False)]), False
                ),
    ...
    def test_udt(self):
        ...
        self.assertRaises(
            ValueError, lambda: _make_type_verifier(ExamplePointUDT())([1.0, 2.0])
        )
        ...
        self.assertRaises(
            ValueError, lambda: _make_type_verifier(PythonOnlyUDT())([1.0, 2.0])
        )
    ...
    def test_nested_udt_in_df(self):
        ...
        df = self.spark.createDataFrame(
            [
                (i % 3, {i % 3: PythonOnlyPoint(float(i + 1), float(i + 1))})
                for i in range(10)
            ],
            schema=schema,
        )
    ...
    def test_cast_to_string_with_udt(self):
        ...
        result = df.select(
            col("point").cast("string"), col("pypoint").cast("string")
        ).head()
    ...
    def test_column_select(self):
        ...
        self.assertEqual(
            [Row(value="1")], df.where(df.key == 1).select(df.value).collect()
        )

    def test_freqItems(self):
        vals = [
            Row(a=1, b=-2.0) if i % 2 == 0 else Row(a=i, b=i * 1.0) for i in range(100)
        ]
    ...
    def test_aggregator(self):
        ...
        g = df.groupBy()
        self.assertEqual(
            [99, 100], sorted(g.agg({"key": "max", "value": "count"}).collect()[0])
        )
    ...
    def test_first_last_ignorenulls(self):
        ...
        df2 = df.select(
            functions.when(df.id % 3 == 0, None).otherwise(df.id).alias("id")
        )
        df3 = df2.select(
    ...
    def test_approxQuantile(self):
        ...
        self.assertRaises(
            ValueError, lambda: df.stat.approxQuantile(123, [0.1, 0.9], 0.1)
        )
        self.assertRaises(
            ValueError, lambda: df.stat.approxQuantile(("a", 123), [0.1, 0.9], 0.1)
        )
        self.assertRaises(
            ValueError, lambda: df.stat.approxQuantile(["a", 123], [0.1, 0.9], 0.1)
        )
    ...
    def test_math_functions(self):
        ...
        assert_close(
            [math.pow(i, 2 * i) for i in range(10)],
            df.select(functions.pow(df.a, df.b)).collect(),
        )
        assert_close(
            [math.pow(i, 2) for i in range(10)],
    ...
    def test_string_functions(self):
        ...
        if sys.version_info.major == 2:
            self.assertRaises(
                TypeError, lambda: df.select(col("name").substr(long(0), long(1)))
            )
    ...
    def test_parse_datatype_string(self):
        ...
        self.assertEqual(
            StructType(
                [StructField("a", IntegerType()), StructField("c", DoubleType())]
            ),
    ...
    def test_stream_trigger(self):
        df = self.spark.readStream.format("text").load(
    ...
    def test_stream_read_options_overwrite(self):
        ...
        df = (
            ...
            .schema(bad_schema)
            .load(
                path="python/test_support/sql/streaming", schema=schema, format="text"
            )
        )
        self.assertTrue(df.isStreaming)
        self.assertEqual(df.schema.simpleString(), "struct<data:string>")
    ...
    def test_stream_status_and_progress(self):
        ...
        try:
            ...
            self.assertTrue(
                "message" in status
                and "isDataAvailable" in status
                and "isTriggerActive" in status
            )
        finally:
            q.stop()
            shutil.rmtree(tmpPath)
    ...
    class ForeachWriterTester:
        ...
        def write_open_event(self, partitionId, epochId):
            self._write_event(
                self.open_events_dir, {"partition": partitionId, "epoch": epochId}
            )
        ...
        def assert_invalid_writer(self, writer, msg=None):
            ...
            try:
                ...
                sq.processAllAvailable()
                self.fail(
                    "invalid writer %s did not fail the query" % str(writer)
                )  # not expected
            except Exception as e:
                if msg:
    ...
    def test_streaming_foreach_with_invalid_writers(self):
        ...
        tester.assert_invalid_writer(
            WriterWithoutProcess(), "does not have a 'process'"
        )
    ...
    def test_merge_type(self):
        ...
        self.assertEqual(
            _merge_type(
                MapType(StringType(), LongType()), MapType(StringType(), LongType())
            ),
            MapType(StringType(), LongType()),
        )
        with self.assertRaisesRegexp(TypeError, "key of map"):
            _merge_type(
                MapType(StringType(), LongType()), MapType(DoubleType(), LongType())
            )
        with self.assertRaisesRegexp(TypeError, "value of map"):
            _merge_type(
                MapType(StringType(), LongType()), MapType(StringType(), DoubleType())
            )

        self.assertEqual(
            _merge_type(
                StructType(
                    [StructField("f1", LongType()), StructField("f2", StringType())]
                ),
                StructType(
                    [StructField("f1", LongType()), StructField("f2", StringType())]
                ),
        ...
        with self.assertRaisesRegexp(TypeError, "field f1"):
            _merge_type(
                ...
                StructType(
                    [StructField("f1", DoubleType()), StructField("f2", StringType())]
                ),
            )
        ...
        with self.assertRaisesRegexp(TypeError, "field f2 in field f1"):
            _merge_type(
                ...
                StructType(
                    [StructField("f1", StructType([StructField("f2", StringType())]))]
                ),
            )
        ...
        with self.assertRaisesRegexp(TypeError, "key of map element in array field f1"):
            _merge_type(
                ...
                StructType(
                    [StructField("f1", ArrayType(MapType(DoubleType(), LongType())))]
                ),
            )
    ...
    def test_filter_with_datetime_timezone(self):
        dt1 = datetime.datetime(
            2015, 4, 17, 23, 1, 2, 3000, tzinfo=UTCOffsetTimezone(0)
        )
        dt2 = datetime.datetime(
            2015, 4, 17, 23, 1, 2, 3000, tzinfo=UTCOffsetTimezone(1)
        )
        row = Row(date=dt1)
        df = self.spark.createDataFrame([row])
    ...
    def test_dropna(self):
        ...
        self.assertEqual(
            self.spark.createDataFrame([("Alice", 50, 80.1)], schema).dropna().count(),
            1,
        )

        # dropping rows with a single null value
        self.assertEqual(
            self.spark.createDataFrame([("Alice", None, 80.1)], schema)
            .dropna()
            .count(),
            0,
        )
        ...
        self.assertEqual(
            self.spark.createDataFrame([(None, None, None)], schema)
            .dropna(how="all")
            .count(),
            0,
        )
    ...
    def test_fillna(self):
        ...
        row = (
            self.spark.createDataFrame([("Alice", 10, 80.1, True)], schema)
            .fillna(50)
            .first()
        )
        ...
        # fillna with int
        row = (
            self.spark.createDataFrame([("Alice", None, None, None)], schema)
            .fillna(50)
            .first()
        )
        ...
        row = (
            self.spark.createDataFrame([Row(a=None), Row(a=True)])
            .fillna({"a": True})
            .first()
        )
    ...
    def test_replace(self):
        ...
        # replace with double
        row = (
            self.spark.createDataFrame([("Alice", 80, 80.0)], schema)
            .replace(80.0, 82.1)
            .first()
        )
    ...
    def test_generic_hints(self):
        ...
        plan = (
            df1.join(df2.hint("broadcast"), "id")._jdf.queryExecution().executedPlan()
        )
    ...
    def test_sample(self):
        ...
        self.assertRaises(
            IllegalArgumentException, lambda: self.spark.range(1).sample(-1.0)
        )
    ...
    def test_toDF_with_schema_string(self):
        ...
        self.assertEqual(
            df.collect(), [Row(key=str(i), value=str(i)) for i in range(100)]
        )
    ...
    def test_join_without_on(self):
        ...
        with self.sql_conf({"spark.sql.crossJoin.enabled": False}):
            self.assertRaises(
                AnalysisException, lambda: df1.join(df2, how="inner").collect()
            )
    ...
    def test_invalid_join_method(self):
        ...
        df2 = self.spark.createDataFrame(
            [("Alice", 80), ("Bob", 90)], ["name", "height"]
        )
        self.assertRaises(
            IllegalArgumentException, lambda: df1.join(df2, how="invalid-join-type")
        )
    ...
    def test_conf(self):
        ...
        self.assertEqual(
            spark.conf.get("spark.sql.sources.partitionOverwriteMode"), "STATIC"
        )
        ...
        self.assertEqual(
            spark.conf.get("spark.sql.sources.partitionOverwriteMode", None), None
        )
    ...
    def test_list_functions(self):
        ...
        functionsDefault = dict(
            (f.name, f) for f in spark.catalog.listFunctions("default")
        )
    ...
    def test_bucketed_write(self):
        ...
        self.assertSetEqual(
            set(data), set(self.spark.table("pyspark_bucket").collect())
        )
        ...
        df.write.bucketBy(2, "x").sortBy("z").mode("overwrite").saveAsTable(
            "pyspark_bucket"
        )
        self.assertEqual(count_bucketed_cols(["x"]), 1)
        self.assertSetEqual(
    ...
    def test_create_dataframe_from_array_of_long(self):
        ...
        df = self.spark.createDataFrame(data)
        self.assertEqual(
            df.first(), Row(longarray=[-9223372036854775808, 0, 9223372036854775807])
        )
    ...
    def test_csv_sampling_ratio(self):
        rdd = self.spark.sparkContext.range(0, 100, 1, 1).map(
            lambda x: "0.1" if x == 1 else str(x)
        )
        schema = (
            self.spark.read.option("inferSchema", True)
    ...
    def test_checking_csv_header(self):
        ...
        shutil.rmtree(path)
        try:
            self.spark.createDataFrame([[1, 1000], [2000, 2]]).toDF(
                "f1", "f2"
    ...
    def test_datasource_with_udf(self):
        ...
        try:
            ...
            filesource_df = (
                self.spark.read.option("inferSchema", True).csv(path).toDF("i")
            )
            datasource_df = (
            ...
            datasource_v2_df = (
                self.spark.read.format(
                    "org.apache.spark.sql.sources.v2.SimpleDataSourceV2"
                )
                .load()
                .toDF("i", "j")
            )
...
class HiveSparkSubmitTests(SparkSubmitTests):
    ...
    def test_hivecontext(self):
        ...
        hive_site_file = self.createTempFile(
            "hive-site.xml",
            (
                """
            |<configuration>
...
class SparkSessionTests(PySparkTestCase):
    ...
    def test_jvm_default_session_already_set(self):
        ...
        try:
            ...
            self.assertTrue(
                jsession.equals(spark._jvm.SparkSession.getDefaultSession().get())
            )
        finally:
            spark.stop()
...
class HiveContextSQLTests(ReusedPySparkTestCase):
    ...
    def test_save_and_load_table(self):
        ...
        schema = StructType([StructField("value", StringType(), True)])
        actual = self.spark.createExternalTable(
            "externalJsonTable",
            source="json",
            schema=schema,
            path=tmpPath,
        ...
        defaultDataSourceName = self.spark.getConf(
            "spark.sql.sources.default", "org.apache.spark.sql.parquet"
        )
        self.spark.sql("SET spark.sql.sources.default=org.apache.spark.sql.json")
    ...
    def test_collect_functions(self):
        ...
        self.assertEqual(
            sorted(
                df.select(functions.collect_set(df.value).alias("r")).collect()[0].r
            ),
            ["1", "2"],
        )
        self.assertEqual(
            sorted(
                df.select(functions.collect_list(df.value).alias("r")).collect()[0].r
            ),
            ["1", "2", "2", "2"],
        )
    ...
    def test_limit_and_take(self):
        ...
        assert_runs_only_one_job_stage_and_task(
            "collect_limit", lambda: df.limit(1).collect()
        )
    ...
    @unittest.skipIf(
        sys.version_info < (3, 3), "Unittest < 3.3 doesn't support mocking"
    )
    def test_unbounded_frames(self):
        ...
        def rows_frame_match():
            return (
                ...
                in df.select(
                    F.count("*").over(
                        window.Window.rowsBetween(-sys.maxsize, sys.maxsize)
                    )
                ).columns[0]
            )
...
class DataTypeVerificationTests(unittest.TestCase):
    def test_verify_type_exception_msg(self):
        self.assertRaisesRegexp(
            ValueError,
            "test_name",
            lambda: _make_type_verifier(StringType(), nullable=False, name="test_name")(
                None
            ),
        )
...
class ArrowTests(ReusedSQLTestCase):
            ...
            with QuietTest(self.sc):
                with self.warnings_lock:
                    with warnings.catch_warnings(record=True) as warns:
                        ...
                        self.assertTrue(
                            "Attempting non-optimization"
                            in _exception_message(user_warns[-1])
                        )
                        self.assertPandasEqual(pdf, pd.DataFrame({"map": [{"a": 1}]}))
    ...
    def test_createDataFrame_respect_session_timezone(self):
        ...
        with self.sql_conf(
            ...
            df_no_arrow_la, df_arrow_la = self._createDataFrame_toggle(
                pdf, schema=self.schema
            )
    ...
    def test_createDataFrame_with_array_type(self):
        ...
        for r in range(len(expected)):
            for e in range(len(expected[r])):
                self.assertTrue(
                    expected[r][e] == result_arrow[r][e]
                    and result[r][e] == result_arrow[r][e]
                )
    ...
    def test_toPandas_with_array_type(self):
        ...
        result_arrow = [
            tuple(list(e) for e in rec) for rec in pdf_arrow.to_records(index=False)
        ]
        for r in range(len(expected)):
            for e in range(len(expected[r])):
                self.assertTrue(
        ...
        with QuietTest(self.sc):
            with self.sql_conf({"spark.sql.execution.arrow.fallback.enabled": True}):
                with warnings.catch_warnings(record=True) as warns:
                    ...
                    warnings.simplefilter("always")
                    df = self.spark.createDataFrame(
                        pd.DataFrame([[{"a": 1}]]), "a: map<string, int>"
                    )
                    # Catch and check the last UserWarning.
                    user_warns = [
                        warn.message
                    ...
                    self.assertEqual(df.collect(), [Row(a={"a": 1})])
...
class EncryptionArrowTests(ArrowTests):
    ...
    def conf(cls):
        return (
            super(EncryptionArrowTests, cls)
            .conf()
...
class PandasUDFTests(ReusedSQLTestCase):
    def test_pandas_udf_basic(self):
        ...
        udf = pandas_udf(
            lambda x: x, "v double", functionType=PandasUDFType.GROUPED_MAP
        )
        self.assertEqual(udf.returnType, StructType([StructField("v", DoubleType())]))
        self.assertEqual(udf.evalType, PythonEvalType.SQL_GROUPED_MAP_PANDAS_UDF)

        udf = pandas_udf(
            lambda x: x, returnType="v double", functionType=PandasUDFType.GROUPED_MAP
        )
        self.assertEqual(udf.returnType, StructType([StructField("v", DoubleType())]))
        self.assertEqual(udf.evalType, PythonEvalType.SQL_GROUPED_MAP_PANDAS_UDF)
    ...
    def test_udf_wrong_arg(self):
        ...
        with QuietTest(self.sc):
            ...
            with self.assertRaisesRegexp(
                ValueError, "0-arg pandas_udfs.*not.*supported"
            ):
            ...
            with self.assertRaisesRegexp(ValueError, "Invalid function"):
                ...
                @pandas_udf(
                    returnType="k int, v double", functionType=PandasUDFType.GROUPED_MAP
                )
                def foo(k, v, w):
                    return k
    ...
    def test_stopiteration_in_udf(self):
        ...
        self.assertRaisesRegexp(
            ...
            exc_message,
            df.groupBy("id")
            .apply(pandas_udf(foo, df.schema, PandasUDFType.GROUPED_MAP))
            .collect,
        )
...
class ScalarPandasUDFTests(ReusedSQLTestCase):
    ...
    def test_pandas_udf_tokenize(self):
        ...
        self.assertEqual(
            [Row(hi=["hi", "boo"]), Row(hi=["bye", "boo"])], result.collect()
        )
    ...
    def test_pandas_udf_nested_arrays(self):
        ...
        tokenize = pandas_udf(
            lambda s: s.apply(lambda str: [str.split(" ")]),
    ...
    def test_vectorized_udf_complex(self):
        ...
        res = df.select(
            add(col("a"), col("b")), power2(col("a")), mul(col("b"), col("c"))
        )
    ...
    def test_vectorized_udf_wrong_return_type(self):
        ...
        with QuietTest(self.sc):
            with self.assertRaisesRegexp(
                NotImplementedError, "Invalid returnType.*scalar Pandas UDF.*MapType"
            ):
    ...
    def test_vectorized_udf_dates(self):
        ...
        def check_data(idx, date, date_copy):
            ...
            for i in range(len(idx)):
                if (is_equal[i] and data[idx[i]][1] is None) or date[i] == data[idx[i]][
                    1
                ]:
                    msgs.append(None)
                else:
    ...
    def test_vectorized_udf_timestamps(self):
        ...
        result = df.withColumn(
            "check_data",
            check_data(col("idx"), col("timestamp"), col("timestamp_copy")),
        ).collect()
    ...
    def test_vectorized_udf_timestamps_respect_session_timezone(self):
        ...
        internal_value = pandas_udf(
            lambda ts: ts.apply(lambda ts: ts.value if ts is not pd.NaT else None),
            LongType(),
        )
        ...
        with self.sql_conf(
            ...
            df_la = df.withColumn(
                "tscopy", f_timestamp_copy(col("timestamp"))
        ...
        with self.sql_conf(
            ...
            result_ny = df_ny.select(
                col("idx"), col("tscopy"), col("internal_value")
            ).collect()
    ...
    @unittest.skipIf(
        sys.version_info[:2] < (3, 5), "Type hints are supported from Python 3.5."
    )
    ...
    def test_datasource_with_udf(self):
        ...
        try:
            ...
            f1 = pandas_udf(lambda x: pd.Series(np.repeat(False, len(x))), "boolean")(
                lit(1)
            )
            f2 = pandas_udf(lambda x: pd.Series(np.repeat(False, len(x))), "boolean")(
                col("i")
            )
...
class GroupedMapPandasUDFTests(ReusedSQLTestCase):
    ...
    def test_supported_types(self):
        ...
        values = [1, 2, 3, 4, 5, 1.1, 2.2, Decimal(1.123), [1, 2, 2], True, "hello"]
        output_fields = [
            ("id", IntegerType()),
            ("byte", ByteType()),
            ("short", ShortType()),
    ...
    def test_array_type_correct(self):
        from pyspark.sql.functions import PandasUDFType, array, col, pandas_udf
    ...
    def test_coerce(self):
        ...
        foo = pandas_udf(
            lambda pdf: pdf, "id long, v double", PandasUDFType.GROUPED_MAP
        )
    ...
    def test_complex_groupby(self):
        ...
        result = (
            df.groupby(col("id") % 2 == 0).apply(normalize).sort("id", "v").toPandas()
        )
        pdf = df.toPandas()
    ...
    def test_wrong_args(self):
        from pyspark.sql.functions import PandasUDFType, pandas_udf, sum, udf
    ...
    def test_timestamp_dst(self):
        ...
        df = self.spark.createDataFrame(dt, "timestamp").toDF("time")
        foo_udf = pandas_udf(
            lambda pdf: pdf, "time timestamp", PandasUDFType.GROUPED_MAP
        )
    ...
    def test_udf_with_key(self):
        ...
        def foo2(key, pdf):
            ...
            return pdf.assign(
                v1=key[0], v2=key[1], v3=pdf.v * key[0], v4=pdf.v + key[1]
            )
    ...
    def test_column_order(self):
        ...
        def rename_pdf(pdf, names):
            pdf.rename(
                columns={old: new for old, new in zip(pd_result.columns, names)},
                inplace=True,
            )
        ...
        int_index_udf = pandas_udf(
            int_index, "id long, u int, v int", PandasUDFType.GROUPED_MAP
        )

        # The UDF result should assign columns by position of integer index
        result = (
            grouped_df.apply(int_index_udf)
            .sort("id", "v")
            .select("id", "u", "v")
            .toPandas()
        )
    ...
    def test_self_join_with_pandas(self):
        ...
        res = df_with_pandas.alias("temp0").join(
            df_with_pandas.alias("temp1"), F.col("temp0.key") == F.col("temp1.key")
        )
    ...
    def test_mixed_scalar_udfs_followed_by_grouby_apply(self):
        ...
        result = df.groupby().apply(
            pandas_udf(
                lambda x: pd.DataFrame([x.sum().sum()]),
                "sum int",
                PandasUDFType.GROUPED_MAP,
            )
        )
...
class GroupedAggPandasUDFTests(ReusedSQLTestCase):
    ...
    def test_basic(self):
        from pyspark.sql.functions import col, lit, mean, sum
        ...
        result2 = (
            df.groupby((col("id") + 1))
            .agg(weighted_mean_udf(df.v, lit(1.0)))
            .sort(df.id + 1)
        )
        expected2 = (
            df.groupby((col("id") + 1))
    ...
    def test_unsupported_types(self):
        ...
        with QuietTest(self.sc):
            with self.assertRaisesRegexp(NotImplementedError, "not supported"):
                ...
                @pandas_udf(
                    MapType(DoubleType(), DoubleType()), PandasUDFType.GROUPED_AGG
                )
                def mean_and_std_udf(v):
                    return {v.mean(): v.std()}
    ...
    def test_complex_groupby(self):
        ...
        result3 = (
            df.groupby(df.id, df.v % 2).agg(sum_udf(df.v)).orderBy(df.id, df.v % 2)
        )
        ...
        result7 = (
            df.groupby(df.v % 2, plus_two(df.id)).agg(sum_udf(df.v)).sort("sum(v)")
        )
    ...
    def test_array_type(self):
        ...
        array_udf = pandas_udf(
            lambda x: [1.0, 2.0], "array<double>", PandasUDFType.GROUPED_AGG
        )
    ...
    def test_invalid_args(self):
        ...
        with QuietTest(self.sc):
            with self.assertRaisesRegexp(AnalysisException, "nor.*aggregate function"):
...
class WindowPandasUDFTests(ReusedSQLTestCase):
    ...
    def test_mixed_sql_and_udf(self):
        ...
        expected4 = df.withColumn("max_v", max(df["v"]).over(w)).withColumn(
            "rank", rank().over(ow)
        )
    ...
    def test_invalid_args(self):
        from pyspark.sql.functions import PandasUDFType, mean, pandas_udf
        ...
        with QuietTest(self.sc):
            with self.assertRaisesRegexp(
                AnalysisException, ".*Only unbounded window frame is supported.*"
            ):
