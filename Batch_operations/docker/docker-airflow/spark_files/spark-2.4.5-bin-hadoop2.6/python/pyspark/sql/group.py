...
class GroupedData(object):
    ...
    def agg(self, *exprs):
        ...
        if len(exprs) == 1 and isinstance(exprs[0], dict):
            ...
            jdf = self._jgd.agg(
                exprs[0]._jc, _to_seq(self.sql_ctx._sc, [c._jc for c in exprs[1:]])
            )
        return DataFrame(jdf, self.sql_ctx)
...
def _test():
    ...
    spark = (
        SparkSession.builder.master("local[4]").appName("sql.group tests").getOrCreate()
    )
    ...
    globs["df5"] = sc.parallelize(
        [
            Row(
                training="expert", sales=Row(course="dotNET", year=2012, earnings=10000)
            ),
            Row(training="junior", sales=Row(course="Java", year=2012, earnings=20000)),
            Row(
                training="expert", sales=Row(course="dotNET", year=2012, earnings=5000)
            ),
            Row(
                training="junior", sales=Row(course="dotNET", year=2013, earnings=48000)
            ),
