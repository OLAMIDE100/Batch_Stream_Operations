...
def _bin_func_op(name, reverse=False, doc="binary function"):
    def _(self, other):
        ...
        jc = (
            ...
            else _create_column_from_literal(other)
        )
...
class Column(object):
    ...
    asc_nulls_first = ignore_unicode_prefix(
        _unary_op("asc_nulls_first", _asc_nulls_first_doc)
    )
    asc_nulls_last = ignore_unicode_prefix(
        _unary_op("asc_nulls_last", _asc_nulls_last_doc)
    )
    desc = ignore_unicode_prefix(_unary_op("desc", _desc_doc))
    desc_nulls_first = ignore_unicode_prefix(
        _unary_op("desc_nulls_first", _desc_nulls_first_doc)
    )
    desc_nulls_last = ignore_unicode_prefix(
        _unary_op("desc_nulls_last", _desc_nulls_last_doc)
    )
    ...
    name = copy_func(
        alias, sinceversion=2.0, doc=":func:`name` is an alias for :func:`alias`."
    )
    ...
    astype = copy_func(
        cast, sinceversion=1.4, doc=":func:`astype` is an alias for :func:`cast`."
    )
