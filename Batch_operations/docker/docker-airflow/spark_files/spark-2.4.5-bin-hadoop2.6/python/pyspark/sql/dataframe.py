...
from pyspark import _NoValue, copy_func, since
...
from pyspark.sql.column import Column, _to_java_column, _to_list, _to_seq
from pyspark.sql.readwriter import DataFrameWriter
from pyspark.sql.streaming import DataStreamWriter
from pyspark.sql.types import *
from pyspark.sql.types import IntegralType, _parse_datatype_json_string
...
class DataFrame(object):
    ...
    def rdd(self):
        ...
        if self._lazy_rdd is None:
            ...
            self._lazy_rdd = RDD(
                jrdd, self.sql_ctx._sc, BatchedSerializer(PickleSerializer())
            )
    ...
    def _repr_html_(self):
        ...
        if self.sql_ctx._conf.isReplEagerEvalEnabled():
            ...
            rows = list(
                _load_from_socket(sock_info, BatchedSerializer(PickleSerializer()))
            )
            head = rows[0]
            row_data = rows[1:]
            ...
            # generate table head
            html += "<tr><th>%s</th></tr>\n" % "</th><th>".join(
                map(lambda x: html_escape(x), head)
            )
            # generate table rows
            for row in row_data:
                html += "<tr><td>%s</td></tr>\n" % "</td><td>".join(
    ...
    def hint(self, name, *parameters):
        ...
        for p in parameters:
            if not isinstance(p, str):
                raise TypeError(
                    "all parameters should be str, got {0} of type {1}".format(
                        p, type(p)
                    )
                )
    ...
    def repartitionByRange(self, numPartitions, *cols):
        ...
        if isinstance(numPartitions, int):
            ...
            cols = (numPartitions,) + cols
            return DataFrame(
                self._jdf.repartitionByRange(self._jcols(*cols)), self.sql_ctx
            )
        else:
    ...
    def randomSplit(self, weights, seed=None):
        ...
        rdd_array = self._jdf.randomSplit(
            _to_list(self.sql_ctx._sc, weights), long(seed)
        )
    ...
    def fillna(self, value, subset=None):
        ...
        if isinstance(value, dict):
            ...
            return DataFrame(
                self._jdf.na().fill(value, self._jseq(subset)), self.sql_ctx
            )
    ...
    def _collectAsArrow(self):
        ...
        with SCCallSiteSync(self._sc):
            ...
            try:
                return list(
                    _load_from_socket((port, auth_secret), ArrowStreamSerializer())
                )
            finally:
    ...
    where = copy_func(
        filter, sinceversion=1.3, doc=":func:`where` is an alias for :func:`filter`."
    )
...
def _test():
    ...
    globs["df5"] = sc.parallelize(
        [
            Row(name="Alice", spy=False, age=10),
            Row(name="Bob", spy=None, age=5),
            Row(name="Mallory", spy=True, age=None),
        ]
    ).toDF()
