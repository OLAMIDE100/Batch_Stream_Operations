    ...
    def awaitTermination(self, timeout=None):
        ...
        if timeout is not None:
            if not isinstance(timeout, (int, float)) or timeout < 0:
                raise ValueError(
                    "timeout must be a positive integer or float. Got %s" % timeout
                )
...
class DataStreamWriter(object):
    ...
    def outputMode(self, outputMode):
        ...
        if not outputMode or type(outputMode) != str or len(outputMode.strip()) == 0:
            raise ValueError(
                "The output mode must be a non-empty string. Got: %s" % outputMode
            )
    ...
    def queryName(self, queryName):
        ...
        if not queryName or type(queryName) != str or len(queryName.strip()) == 0:
            raise ValueError(
                "The queryName must be a non-empty string. Got: %s" % queryName
            )
    ...
    def foreach(self, f):
        ...
        jForeachWriter = self._spark._sc._jvm.org.apache.spark.sql.execution.python.PythonForeachWriter(
