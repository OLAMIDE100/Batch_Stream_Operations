...
from pyspark.rdd import PythonEvalType, ignore_unicode_prefix
...
def _create_function(name, doc=""):
    ...
    def _(col):
        ...
        jc = getattr(sc._jvm.functions, name)(
            col._jc if isinstance(col, Column) else col
        )
        return Column(jc)
...
def covar_pop(col1, col2):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.covar_pop(_to_java_column(col1), _to_java_column(col2))
    )
...
def covar_samp(col1, col2):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.covar_samp(_to_java_column(col1), _to_java_column(col2))
    )
...
def countDistinct(col, *cols):
    ...
    """
    sc = SparkContext._active_spark_context
    jc = sc._jvm.functions.countDistinct(
...
def datediff(end, start):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.datediff(_to_java_column(end), _to_java_column(start))
    )
...
def months_between(date1, date2, roundOff=True):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.months_between(
...
def format_string(format, *cols):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.format_string(format, _to_seq(sc, cols, _to_java_column))
    )
...
def translate(srcCol, matching, replace):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.translate(_to_java_column(srcCol), matching, replace)
    )
...
def map_from_arrays(col1, col2):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.map_from_arrays(_to_java_column(col1), _to_java_column(col2))
    )
...
def arrays_overlap(a1, a2):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.arrays_overlap(_to_java_column(a1), _to_java_column(a2))
    )
...
def element_at(col, extraction):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.element_at(_to_java_column(col), lit(extraction)._jc)
    )  # noqa: F821 'lit' is dynamically defined.
...
def array_intersect(col1, col2):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.array_intersect(_to_java_column(col1), _to_java_column(col2))
    )
...
def array_union(col1, col2):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.array_union(_to_java_column(col1), _to_java_column(col2))
    )
...
def array_except(col1, col2):
    ...
    sc = SparkContext._active_spark_context
    return Column(
        sc._jvm.functions.array_except(_to_java_column(col1), _to_java_column(col2))
    )
...
def sequence(start, stop, step=None):
    ...
    if step is None:
        return Column(
            sc._jvm.functions.sequence(_to_java_column(start), _to_java_column(stop))
        )
    else:
        return Column(
...
def udf(f=None, returnType=StringType()):
    ...
    if f is None or isinstance(f, (str, DataType)):
        ...
        return _create_udf(
            f=f, returnType=returnType, evalType=PythonEvalType.SQL_BATCHED_UDF
        )
...
def pandas_udf(f=None, returnType=None, functionType=None):
    ...
    if is_decorator:
        return functools.partial(
            _create_udf, returnType=return_type, evalType=eval_type
        )
    else:
...
def _test():
    ...
    globs["sc"] = sc
    globs["spark"] = spark
    globs["df"] = spark.createDataFrame(
