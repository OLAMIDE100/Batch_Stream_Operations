...
class FPGrowthModel(JavaModelWrapper, JavaSaveable, JavaLoader):
    ...
    def freqItemsets(self):
        ...
        """
        return self.call("getFreqItemsets").map(
...
class FPGrowth(object):
    ...
    def train(cls, data, minSupport=0.3, numPartitions=-1):
        ...
        """
        model = callMLlibFunc(
            "trainFPGrowthModel", data, float(minSupport), int(numPartitions)
        )
...
class PrefixSpanModel(JavaModelWrapper):
    ...
    def freqSequences(self):
        ...
        return self.call("getFreqSequences").map(
...
class PrefixSpan(object):
    ...
    @since("1.6.0")
    def train(
        cls, data, minSupport=0.1, maxPatternLength=10, maxLocalProjDBSize=32000000
    ):
        """
...
def _test():
    ...
    spark = (
        SparkSession.builder.master("local[4]").appName("mllib.fpm tests").getOrCreate()
    )
