...
class BisectingKMeans(object):
    ...
    @since("2.0.0")
    def train(
        self, rdd, k=4, maxIterations=20, minDivisibleClusterSize=1.0, seed=-1888008604
    ):
        """
...
class GaussianMixtureModel(JavaModelWrapper, JavaSaveable, JavaLoader):
    ...
    def load(cls, sc, path):
        ...
        wrapper = sc._jvm.org.apache.spark.mllib.api.python.GaussianMixtureModelWrapper(
            model
        )
        return cls(wrapper)
...
class GaussianMixture(object):
    ...
    def train(
        ...
        if initialModel is not None:
            ...
            initialModelMu = [
                initialModel.gaussians[i].mu for i in range(initialModel.k)
            ]
            initialModelSigma = [
                initialModel.gaussians[i].sigma for i in range(initialModel.k)
            ]
        java_model = callMLlibFunc(
