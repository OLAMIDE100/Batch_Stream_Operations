...
def _vector_size(v):
    ...
    if isinstance(v, Vector):
        ...
        if v.ndim == 1 or (v.ndim == 2 and v.shape[1] == 1):
            ...
            raise ValueError(
                "Cannot treat an ndarray of shape %s as a vector" % str(v.shape)
            )
    ...
    def __eq__(self, other):
        if isinstance(other, DenseVector):
            ...
            return Vectors._equals(
                list(xrange(len(self))), self.array, other.indices, other.values
            )
        return False
    ...
    def __init__(self, size, *args):
        ...
        if len(args) == 1:
            ...
            assert len(self.indices) == len(
                self.values
            ), "index and value arrays not same length"
    ...
    def __eq__(self, other):
        if isinstance(other, SparseVector):
            ...
            if self.size != len(other):
                return False
            return Vectors._equals(
                self.indices, self.values, list(xrange(len(other))), other.array
            )
        return False
...
class Vectors(object):
    ...
    def _equals(v1_indices, v1_values, v2_indices, v2_values):
        ...
        while all_equal:
            ...
            all_equal = (
                v1_indices[k1] == v2_indices[k2] and v1_values[k1] == v2_values[k2]
            )
            k1 += 1
            k2 += 1
        return all_equal
...
class DenseMatrix(Matrix):
    ...
    def toSparse(self):
        ...
        colPtrs = np.cumsum(
            np.hstack((0, colCounts, np.zeros(self.numCols - colCounts.size)))
        )
    ...
    def asML(self):
        ...
        .. versionadded:: 2.0.0
        """
        return newlinalg.DenseMatrix(
            self.numRows, self.numCols, self.values, self.isTransposed
        )
    ...
    def __getitem__(self, indices):
        ...
        if j >= self.numCols or j < 0:
            raise IndexError(
                "Column index %d is out of range [0, %d)" % (j, self.numCols)
            )
    ...
    def __eq__(self, other):
        if (
            not isinstance(other, DenseMatrix)
            or self.numRows != other.numRows
            or self.numCols != other.numCols
        ):
            return False
...
class SparseMatrix(Matrix):
    ...
    def __init__(
        self, numRows, numCols, colPtrs, rowIndices, values, isTransposed=False
    ):
        Matrix.__init__(self, numRows, numCols, isTransposed)
        ...
        if self.isTransposed:
            if self.colPtrs.size != numRows + 1:
                raise ValueError(
                    "Expected colPtrs of size %d, got %d."
    ...
    def __str__(self):
        ...
        for i, (rowInd, value) in enumerate(zipindval):
            ...
            if self.isTransposed:
                smlist.append(
                    "({0},{1}) {2}".format(cur_col, rowInd, _format_float(value))
                )
            else:
                smlist.append(
                    "({0},{1}) {2}".format(rowInd, cur_col, _format_float(value))
                )
        spstr += "\n".join(smlist)
