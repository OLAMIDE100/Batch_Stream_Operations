...
class StandardScaler(object):
    ...
    def fit(self, dataset):
        ...
        jmodel = callMLlibFunc(
            "fitStandardScaler", self.withMean, self.withStd, dataset
        )
...
class IDF(object):
    ...
    def fit(self, dataset):
        ...
        jmodel = callMLlibFunc(
            "fitIDF", self.minDocFreq, dataset.map(_convert_to_vector)
        )
...
class Word2VecModel(JavaVectorTransformer, JavaSaveable, JavaLoader):
    ...
    def load(cls, sc, path):
        ...
        """
        jmodel = sc._jvm.org.apache.spark.mllib.feature.Word2VecModel.load(
            sc._jsc.sc(), path
        )
