...
class RowMatrix(DistributedMatrix):
    ...
    def __init__(self, rows, numRows=0, numCols=0):
        ...
        if isinstance(rows, RDD):
            ...
            java_matrix = callMLlibFunc(
                "createRowMatrix", rows, long(numRows), int(numCols)
            )
        elif (
            isinstance(rows, JavaObject)
    ...
    def computeColumnSummaryStatistics(self):
        ...
        array([ 2.5,  3.5,  4.5])
        """
        java_col_stats = self._java_matrix_wrapper.call(
    ...
    def columnSimilarities(self, threshold=0.0):
        ...
        0.91914503...
        """
        java_sims_mat = self._java_matrix_wrapper.call(
            "columnSimilarities", float(threshold)
        )
...
class CoordinateMatrix(DistributedMatrix):
    ...
    def entries(self):
        ...
        entries_df = callMLlibFunc(
            "getMatrixEntries", self._java_matrix_wrapper._java_model
        )
...
def _convert_to_matrix_block_tuple(block):
    if (
        ...
        raise TypeError(
            "Cannot convert type %s into a sub-matrix block tuple" % type(block)
        )
...
class BlockMatrix(DistributedMatrix):
    ...
    def blocks(self):
        ...
        blocks_df = callMLlibFunc(
            "getMatrixBlocks", self._java_matrix_wrapper._java_model
        )
    ...
    def persist(self, storageLevel):
        ...
        if not isinstance(storageLevel, StorageLevel):
            raise TypeError(
                "`storageLevel` should be a StorageLevel, got %s" % type(storageLevel)
            )
    ...
    def subtract(self, other):
        ...
        other_java_block_matrix = other._java_matrix_wrapper._java_model
        java_block_matrix = self._java_matrix_wrapper.call(
            "subtract", other_java_block_matrix
        )
        return BlockMatrix(java_block_matrix, self.rowsPerBlock, self.colsPerBlock)
    ...
    def multiply(self, other):
        ...
        other_java_block_matrix = other._java_matrix_wrapper._java_model
        java_block_matrix = self._java_matrix_wrapper.call(
            "multiply", other_java_block_matrix
        )
        return BlockMatrix(java_block_matrix, self.rowsPerBlock, self.colsPerBlock)
