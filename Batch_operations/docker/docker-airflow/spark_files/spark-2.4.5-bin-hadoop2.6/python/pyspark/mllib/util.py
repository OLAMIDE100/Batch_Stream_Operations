...
class MLUtils(object):
    ...
    def loadLibSVMFile(sc, path, numFeatures=-1, minPartitions=None, multiclass=None):
        ...
        if numFeatures <= 0:
            parsed.cache()
            numFeatures = (
                parsed.map(lambda x: -1 if x[1].size == 0 else x[1][-1]).reduce(max) + 1
            )
        return parsed.map(
            lambda x: LabeledPoint(x[0], Vectors.sparse(numFeatures, x[1], x[2]))
        )
    ...
    def convertVectorColumnsToML(dataset, *cols):
        ...
        if not isinstance(dataset, DataFrame):
            raise TypeError(
                "Input dataset must be a DataFrame but got {}.".format(type(dataset))
            )
