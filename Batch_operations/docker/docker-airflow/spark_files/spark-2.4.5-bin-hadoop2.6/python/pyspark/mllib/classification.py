...
class LogisticRegressionModel(LinearClassificationModel):
        ...
        if self.numClasses == 2:
            ...
            if x.size + 1 == self._dataWithBiasSize:
                for i in range(0, self._numClasses - 1):
                    margin = (
                        x.dot(self._weightsMatrix[i][0 : x.size])
    ...
    def load(cls, sc, path):
        ...
        """
        java_model = (
            sc._jvm.org.apache.spark.mllib.classification.LogisticRegressionModel.load(
                sc._jsc.sc(), path
            )
        )
...
class LogisticRegressionWithSGD(object):
    ...
    def train(
        ...
        return _regression_train_wrapper(
            train, LogisticRegressionModel, data, initialWeights
        )
    ...
    def train(
        ...
        if initialWeights is None:
            if numClasses == 2:
                ...
                if intercept:
                    initialWeights = (
                        [0.0] * (len(data.first().features) + 1) * (numClasses - 1)
                    )
                else:
                    initialWeights = (
                        [0.0] * len(data.first().features) * (numClasses - 1)
                    )
        return _regression_train_wrapper(
            train, LogisticRegressionModel, data, initialWeights
        )
