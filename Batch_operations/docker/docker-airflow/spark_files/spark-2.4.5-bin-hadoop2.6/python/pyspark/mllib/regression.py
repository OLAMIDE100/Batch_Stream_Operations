...
class LinearRegressionModel(LinearRegressionModelBase):
    ...
    def load(cls, sc, path):
        ...
        java_model = (
            sc._jvm.org.apache.spark.mllib.regression.LinearRegressionModel.load(
                sc._jsc.sc(), path
            )
        )
...
def _regression_train_wrapper(train_func, modelClass, data, initial_weights):
    ...
    if not isinstance(first, LabeledPoint):
        raise TypeError(
            "data should be an RDD of LabeledPoint, but got %s" % type(first)
        )
...
class LinearRegressionWithSGD(object):
    ...
    def train(
        ...
        return _regression_train_wrapper(
            train, LinearRegressionModel, data, initialWeights
        )
...
class RidgeRegressionModel(LinearRegressionModelBase):
    ...
    def load(cls, sc, path):
        ...
        java_model = (
            sc._jvm.org.apache.spark.mllib.regression.RidgeRegressionModel.load(
                sc._jsc.sc(), path
            )
        )
        weights = _java2py(sc, java_model.weights())
...
class RidgeRegressionWithSGD(object):
    ...
    def train(
        ...
        return _regression_train_wrapper(
            train, RidgeRegressionModel, data, initialWeights
        )
...
class StreamingLinearAlgorithm(object):
    ...
    def _validate(self, dstream):
        ...
        if not self._model:
            raise ValueError("Model must be intialized using setInitialWeights")
