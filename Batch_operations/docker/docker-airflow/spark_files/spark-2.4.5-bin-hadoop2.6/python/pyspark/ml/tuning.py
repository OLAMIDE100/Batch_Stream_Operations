...
class ParamGridBuilder(object):
    ...
    def build(self):
        ...
        return [
            dict(to_key_value_pairs(keys, prod))
            for prod in itertools.product(*grid_values)
        ]
...
class CrossValidator(
    ...
    def _from_java(cls, java_stage):
        ...
        estimator, epms, evaluator = super(CrossValidator, cls)._from_java_impl(
            java_stage
        )
    ...
    def _to_java(self):
        ...
        _java_obj = JavaParams._new_java_obj(
            "org.apache.spark.ml.tuning.CrossValidator", self.uid
        )
...
class CrossValidatorModel(Model, ValidatorParams, MLReadable, MLWritable):
    ...
    def _from_java(cls, java_stage):
        ...
        estimator, epms, evaluator = super(CrossValidatorModel, cls)._from_java_impl(
            java_stage
        )
...
class TrainValidationSplit(
    ...
    def _fit(self, dataset):
        ...
        return self._copyValues(
            TrainValidationSplitModel(bestModel, metrics, subModels)
        )
    ...
    def _from_java(cls, java_stage):
        ...
        estimator, epms, evaluator = super(TrainValidationSplit, cls)._from_java_impl(
            java_stage
        )
...
class TrainValidationSplitModel(Model, ValidatorParams, MLReadable, MLWritable):
    ...
    def _from_java(cls, java_stage):
        ...
        # Load information from java_stage to the instance.
        bestModel = JavaParams._from_java(java_stage.bestModel())
        estimator, epms, evaluator = super(
            TrainValidationSplitModel, cls
    ...
    def _to_java(self):
        ...
        estimator, epms, evaluator = super(
            TrainValidationSplitModel, self
        )._to_java_impl()
...
if __name__ == "__main__":
    ...
    spark = (
        SparkSession.builder.master("local[2]").appName("ml.tuning tests").getOrCreate()
    )
