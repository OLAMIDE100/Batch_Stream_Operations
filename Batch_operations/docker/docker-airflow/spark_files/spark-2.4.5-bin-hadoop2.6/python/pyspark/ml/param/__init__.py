...
class Param(object):
    ...
    def __init__(self, parent, name, doc, typeConverter=None):
        if not isinstance(parent, Identifiable):
            raise TypeError(
                "Parent must be an Identifiable but got type %s." % type(parent)
            )
...
class TypeConverters(object):
    ...
    def _can_convert_to_list(value):
        vtype = type(value)
        return vtype in [list, np.ndarray, tuple, xrange, array.array] or isinstance(
            value, Vector
        )
    ...
    def _can_convert_to_string(value):
        vtype = type(value)
        return isinstance(value, basestring) or vtype in [
            np.unicode_,
            np.string_,
            np.str_,
        ]
    ...
    def toBoolean(value):
        ...
        if type(value) == bool:
            ...
            raise TypeError(
                "Boolean Param requires value of type bool. Found %s." % type(value)
            )
...
class Params(Identifiable):
    ...
    def _copy_params(self):
        ...
        src_params = list(
            filter(lambda nameAttr: isinstance(nameAttr[1], Param), src_name_attrs)
        )
    ...
    def set(self, param, value):
        ...
        try:
            ...
            raise ValueError(
                'Invalid param value given for param "%s". %s' % (param.name, e)
            )
    ...
    def _copyValues(self, to, extra=None):
        ...
        for param in self.params:
            ...
            if param in self._defaultParamMap and to.hasParam(param.name):
                to._defaultParamMap[to.getParam(param.name)] = self._defaultParamMap[
                    param
                ]
