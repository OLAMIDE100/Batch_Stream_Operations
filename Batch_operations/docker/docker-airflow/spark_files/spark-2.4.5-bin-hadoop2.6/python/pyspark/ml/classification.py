...
class LogisticRegression(
    ...
    def __init__(
        ...
        self._setDefault(
            maxIter=100, regParam=0.0, tol=1e-6, threshold=0.5, family="auto"
        )
        kwargs = self._input_kwargs
        self.setParams(**kwargs)
...
class RandomForestClassificationModel(
    ...
    def trees(self):
        ...
        return [
            DecisionTreeClassificationModel(m) for m in list(self._call_java("trees"))
        ]
...
class MultilayerPerceptronClassifier(
    ...
    def __init__(
        ...
        self._setDefault(
            maxIter=100, tol=1e-6, blockSize=128, stepSize=0.03, solver="l-bfgs"
        )
        kwargs = self._input_kwargs
        self.setParams(**kwargs)
...
class OneVsRest(
    ...
    def _fit(self, dataset):
        ...
        numClasses = (
            int(dataset.agg({labelCol: "max"}).head()["max(" + labelCol + ")"]) + 1
        )
        ...
        handlePersistence = dataset.storageLevel == StorageLevel(
        ...
        for pair in javaParamMap.toList():
            ...
            if self.hasParam(str(param.name())):
                if param.name() == "classifier":
                    paramMap[self.getParam(param.name())] = JavaParams._from_java(
                        pair.value()
                    )
                else:
    ...
    def _transform(self, dataset):
        ...
        for index, model in enumerate(self.models):
            ...
            updatedDataset = transformedDataset.withColumn(
                tmpColName,
                updateUDF(
                    transformedDataset[accColName], transformedDataset[rawPredictionCol]
                ),
            )
