...
class _ImageSchema(object):
    ...
    def imageFields(self):
        ...
        if self._imageFields is None:
            ...
            self._imageFields = list(
                ctx._jvm.org.apache.spark.ml.image.ImageSchema.imageFields()
            )
    ...
    def toImage(self, array, origin=""):
        ...
        return _create_row(
            self.imageFields, [origin, height, width, nChannels, mode, data]
        )
    ...
    def readImages(
        ...
        jresult = image_schema.readImages(
            path,
            jsession,
            recursive,
            numPartitions,
...
def _test():
    ...
    spark = (
        SparkSession.builder.master("local[2]").appName("ml.image tests").getOrCreate()
    )
    globs["spark"] = spark
