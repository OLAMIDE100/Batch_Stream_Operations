...
class MergerTests(unittest.TestCase):
    ...
    def test_small_dataset(self):
        ...
        self.assertEqual(sum(sum(v) for k, v in m.items()), sum(xrange(self.N)))
    ...
    def test_medium_dataset(self):
        ...
        self.assertTrue(m.spills >= 1)
        self.assertEqual(sum(sum(v) for k, v in m.items()), sum(xrange(self.N)) * 3)
    ...
    def test_stopiteration_is_raised(self):
        ...
        # wrong merge value
        m = ExternalMerger(
            Aggregator(legit_create_combiner, stopit, legit_merge_combiners), 20
        )
        with self.assertRaises((Py4JJavaError, RuntimeError)) as cm:
            m.mergeValues(data)
...
class SorterTests(unittest.TestCase):
    def test_in_memory_sort(self):
        ...
        self.assertEqual(
            sorted(l, key=lambda x: -x), list(sorter.sorted(l, key=lambda x: -x))
        )
        self.assertEqual(
...
class SerializationTestCase(unittest.TestCase):
    ...
    def test_compressed_serializer(self):
        ...
        io.seek(0)
        self.assertEqual(
            ["abc", "123", range(5)] + list(range(1000)), list(ser.load_stream(io))
        )
        io.close()
...
class AddFileTests(PySparkTestCase):
    ...
    def test_overwrite_system_module(self):
        self.sc.addPyFile(
            os.path.join(SPARK_HOME, "python/test_support/SimpleHTTPServer.py")
        )
        ...
        self.assertEqual(
            ["My Server"], self.sc.parallelize(range(1)).map(func).collect()
        )
...
class TaskContextTests(PySparkTestCase):
    ...
    def test_get_local_property(self):
        ...
        try:
            ...
            prop1 = rdd.map(
                lambda _: TaskContext.get().getLocalProperty(key)
            ).collect()[0]
            self.assertEqual(prop1, value)
            prop2 = rdd.map(
                lambda _: TaskContext.get().getLocalProperty("otherkey")
            ).collect()[0]
...
class RDDTests(ReusedPySparkTestCase):
    ...
    def test_save_as_textfile_with_unicode(self):
        ...
        raw_contents = b"".join(
            open(p, "rb").read() for p in glob(tempFile.name + "/part-0000*")
        )
    ...
    def test_zip_chaining(self):
        ...
        self.assertSetEqual(
            set(rdd.zip(rdd).zip(rdd).collect()), set([((x, x), x) for x in "abc"])
        )
        self.assertSetEqual(
            set(rdd.zip(rdd.zip(rdd)).collect()), set([(x, (x, x)) for x in "abc"])
        )
    ...
    def test_zip_with_different_serializers(self):
        ...
        self.assertEqual(
            a.zip(b).collect(), [(0, 100), (1, 101), (2, 102), (3, 103), (4, 104)]
        )
    ...
    def test_count_approx_distinct(self):
        ...
        self.assertTrue(
            950 < rdd.map(lambda x: (x, -x)).countApproxDistinct(0.03) < 1050
        )
    ...
    def test_narrow_dependency_in_join(self):
        ...
        self.assertEqual(
            rdd.getNumPartitions() + 2, parted.union(rdd).getNumPartitions()
        )
        self.assertEqual(
            rdd.getNumPartitions() + 2, rdd.union(parted).getNumPartitions()
        )
...
class InputFormatTests(ReusedPySparkTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sc._jvm.WriteInputFormatTestDataGenerator.generateData(
            cls.tempdir.name, cls.sc._jsc
        )
    ...
    def test_sequencefiles(self):
        ...
        et = [(1, ()), (2, (3.0, 4.0, 5.0)), (3, (4.0, 5.0, 6.0))]
        ...
        clazz = sorted(
            self.sc.sequenceFile(
                basepath + "/sftestdata/sfclass/",
                "org.apache.hadoop.io.Text",
                "org.apache.spark.api.python.TestWritable",
            ).collect()
        )
        cname = "org.apache.spark.api.python.TestWritable"
        ec = [
            ("1", {"__class__": cname, "double": 1.0, "int": 1, "str": "test1"}),
            ("2", {"__class__": cname, "double": 2.3, "int": 2, "str": "test2"}),
            ("3", {"__class__": cname, "double": 3.1, "int": 3, "str": "test3"}),
            ("4", {"__class__": cname, "double": 4.2, "int": 4, "str": "test4"}),
            ("5", {"__class__": cname, "double": 5.5, "int": 5, "str": "test56"}),
        ]
        self.assertEqual(clazz, ec)

        unbatched_clazz = sorted(
            self.sc.sequenceFile(
                basepath + "/sftestdata/sfclass/",
                "org.apache.hadoop.io.Text",
                "org.apache.spark.api.python.TestWritable",
...
class OutputFormatTests(ReusedPySparkTestCase):
    ...
    def test_oldhadoop(self):
        ...
        input_conf = {
            "mapreduce.input.fileinputformat.inputdir": basepath + "/olddataset/"
        }
    ...
    def test_newhadoop(self):
        ...
        input_conf = {
            "mapreduce.input.fileinputformat.inputdir": basepath + "/newdataset/"
        }
        new_dataset = sorted(
...
class SparkSubmitTests(unittest.TestCase):
    ...
    def test_module_dependency_on_cluster(self):
        ...
        proc = subprocess.Popen(
            self.sparkSubmit
            + ["--py-files", zip, "--master", "local-cluster[1,1,1024]", script],
...
class ConfTests(unittest.TestCase):
    def test_memory_conf(self):
        ...
        for memory in memoryList:
            sc = SparkContext(
                conf=SparkConf().set("spark.python.worker.memory", memory)
            )
            l = list(range(1024))
            random.shuffle(l)
            rdd = sc.parallelize(l, 4)
