...
class DStream(object):
    ...
    def combineByKey(
        self, createCombiner, mergeValue, mergeCombiners, numPartitions=None
    ):
        """
    ...
    def transformWith(self, func, other, keepSerializer=False):
        ...
        jfunc = TransformFunction(
            self._sc, func, self._jrdd_deserializer, other._jrdd_deserializer
        )
        dstream = self._sc._jvm.PythonTransformed2DStream(
            self._jdstream.dstream(), other._jdstream.dstream(), jfunc
        )
        jrdd_serializer = (
            self._jrdd_deserializer if keepSerializer else self._sc.serializer
        )
    ...
    def rightOuterJoin(self, other, numPartitions=None):
        ...
        return self.transformWith(
            lambda a, b: a.rightOuterJoin(b, numPartitions), other
        )
    ...
    def _validate_window_param(self, window, slide):
        ...
        if int(window * 1000) % duration != 0:
            raise ValueError(
                "windowDuration must be multiple of the slide duration (%d ms)"
                % duration
            )
        if slide and int(slide * 1000) % duration != 0:
            raise ValueError(
                "slideDuration must be multiple of the slide duration (%d ms)"
                % duration
            )
    ...
    def reduceByKeyAndWindow(
        ...
        if invFunc:
            ...
            jreduceFunc = TransformFunction(
                self._sc, reduceFunc, reduced._jrdd_deserializer
            )
            jinvReduceFunc = TransformFunction(
                self._sc, invReduceFunc, reduced._jrdd_deserializer
            )
            if slideDuration is None:
    ...
    def updateStateByKey(self, updateFunc, numPartitions=None, initialRDD=None):
        ...
        def reduceFunc(t, a, b):
            if a is None:
                ...
                g = g.mapValues(
                    lambda ab: (list(ab[1]), list(ab[0])[0] if len(ab[0]) else None)
                )
...
class TransformedDStream(DStream):
    ...
    def __init__(self, prev, func):
        ...
        if (
            type(prev) is TransformedDStream
            and not prev.is_cached
            and not prev.is_checkpointed
        ):
            prev_func = prev.func
