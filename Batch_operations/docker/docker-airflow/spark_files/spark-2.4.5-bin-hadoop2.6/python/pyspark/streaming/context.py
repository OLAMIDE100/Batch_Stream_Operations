...
class StreamingContext(object):
    ...
    def getOrCreate(cls, checkpointPath, setupFunc):
        ...
        ssc_option = gw.jvm.StreamingContextPythonHelper().tryRecoverFromCheckpoint(
            checkpointPath
        )
    ...
    def socketTextStream(
        ...
        return DStream(
            self._jssc.socketTextStream(hostname, port, jlevel),
            self,
            UTF8Deserializer(),
        )
    ...
    def binaryRecordsStream(self, directory, recordLength):
        ...
        """
        return DStream(
            self._jssc.binaryRecordsStream(directory, recordLength),
            self,
            NoOpSerializer(),
        )
    ...
    def union(self, *dstreams):
        ...
        return DStream(
            self._jssc.union(first._jdstream, jrest), self, first._jrdd_deserializer
        )
