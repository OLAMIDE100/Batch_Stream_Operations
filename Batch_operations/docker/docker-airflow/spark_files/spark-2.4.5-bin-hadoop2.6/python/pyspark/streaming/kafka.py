...
class KafkaUtils(object):
    ...
    def createRDD(
        ...
        jleaders = dict(
            [
                (k._jTopicAndPartition(helper), v._jBroker(helper))
...
class OffsetRange(object):
    ...
    def _jOffsetRange(self, helper):
        return helper.createOffsetRange(
...
class KafkaDStream(DStream):
    ...
    def foreachRDD(self, func):
        ...
        jfunc = TransformFunction(self._sc, func, self._jrdd_deserializer).rdd_wrapper(
            lambda jrdd, ctx, ser: KafkaRDD(jrdd, ctx, ser)
        )
...
class KafkaTransformedDStream(TransformedDStream):
    ...
    def _jdstream(self):
        ...
        jfunc = TransformFunction(
            self._sc, self.func, self.prev._jrdd_deserializer
        ).rdd_wrapper(lambda jrdd, ctx, ser: KafkaRDD(jrdd, ctx, ser))
        dstream = self._sc._jvm.PythonTransformedDStream(
...
class KafkaMessageAndMetadata(object):
    ...
    def __str__(self):
        return (
            "KafkaMessageAndMetadata(topic: %s, partition: %d, offset: %d, key and message...)"
