...
from pyspark.context import RDD, SparkConf, SparkContext
...
class BasicOperationTests(PySparkStreamingTestCase):
    ...
    def test_flatMap(self):
        ...
        expected = [
            list(chain.from_iterable((map(lambda y: [y, y * 2], x)))) for x in input
        ]
        self._test_func(input, func, expected)
...
class WindowFunctionTests(PySparkStreamingTestCase):
    ...
    def test_reduce_by_invalid_window(self):
        ...
        self.assertRaises(
            ValueError, lambda: d1.reduceByKeyAndWindow(None, None, 0.1, 0.1)
        )
        self.assertRaises(
            ValueError, lambda: d1.reduceByKeyAndWindow(None, None, 1, 0.1)
        )
...
class StreamingContextTests(PySparkStreamingTestCase):
    ...
    def test_transform_pairrdd(self):
        ...
        dstream = self.ssc.queueStream([[1], [2], [3]]).transform(
            lambda rdd: rdd.cartesian(rdd)
        )
...
class CheckpointTests(unittest.TestCase):
    ...
    def test_transform_function_serializer_failure(self):
        ...
        self.fail(
            "using SparkContext in process should fail because it's not Serializable"
        )
...
class KafkaStreamTests(PySparkStreamingTestCase):
    ...
    def setUp(self):
        ...
        self._kafkaTestUtils = (
            self.ssc._jvm.org.apache.spark.streaming.kafka.KafkaTestUtils()
        )
    ...
    def _validateStreamResult(self, sendData, stream):
        ...
        for i in chain.from_iterable(
            self._collect(stream.map(lambda x: x[1]), sum(sendData.values()))
        ):
    ...
    def test_kafka_direct_stream_from_offset(self):
        ...
        stream = KafkaUtils.createDirectStream(
            self.ssc, [topic], kafkaParams, fromOffsets
        )
        self._validateStreamResult(sendData, stream)
    ...
    def test_kafka_rdd_message_handler(self):
        ...
        rdd = KafkaUtils.createRDD(
            self.sc, kafkaParams, offsetRanges, messageHandler=getKeyAndDoubleMessage
        )
    ...
    def test_kafka_direct_stream_message_handler(self):
        ...
        stream = KafkaUtils.createDirectStream(
            self.ssc, [topic], kafkaParams, messageHandler=getKeyAndDoubleMessage
        )
...
class FlumeStreamTests(PySparkStreamingTestCase):
    ...
    def _startContext(self, n, compressed):
        ...
        dstream = FlumeUtils.createStream(
            self.ssc,
            "localhost",
    ...
    def _writeAndVerify(self, ports):
        ...
        try:
            ...
            dstream = FlumeUtils.createPollingStream(
                ssc, addresses, maxBatchSize=self._utils.eventsPerBatch(), parallelism=5
            )
            outputBuffer = []
...
def search_jar(dir, name_prefix):
    ...
    jars = glob.glob(
        os.path.join(dir, "target/scala-*/" + name_prefix + "-*.jar")
    ) + glob.glob(  # sbt build
        os.path.join(dir, "target/" + name_prefix + "_*.jar")
    )  # maven build
...
def search_kafka_assembly_jar():
    ...
    if not jars:
        ...
        raise Exception(
            (
                "Found multiple Spark Streaming Kafka assembly JARs: %s; please "
                "remove all but one"
            )
            % (", ".join(jars))
        )
...
def search_flume_assembly_jar():
    ...
    if not jars:
        ...
        raise Exception(
            (
                "Found multiple Spark Streaming Flume assembly JARs: %s; please "
                "remove all but one"
            )
            % (", ".join(jars))
        )
...
def search_kinesis_asl_assembly_jar():
    jars = search_jar(
        _kinesis_asl_assembly_dir(), "spark-streaming-kinesis-asl-assembly"
    )
    if not jars:
        return None
    elif len(jars) > 1:
        raise Exception(
            (
                "Found multiple Spark Streaming Kinesis ASL assembly JARs: %s; please "
                "remove all but one"
            )
            % (", ".join(jars))
        )
...
if __name__ == "__main__":
    ...
    jars_list = [
        j
        for j in (kafka_assembly_jar, flume_assembly_jar, kinesis_asl_assembly_jar)
        if j
    ]
    jars = ",".join(jars_list)
    ...
    if kafka08_jar_present:
        ...
        sys.stderr.write(
            "Skipping kafka-0-8 tests as the optional kafka-0-8 profile was "
    ...
    for testcase in testcases:
        ...
        if xmlrunner:
            result = xmlrunner.XMLTestRunner(
                output="target/test-reports", verbosity=2
            ).run(tests)
