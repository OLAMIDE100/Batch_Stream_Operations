...
from pyspark.util import _exception_message, fail_on_stopiteration
...
class RDD(object):
    ...
    def __init__(
        self, jrdd, ctx, jrdd_deserializer=AutoBatchedSerializer(PickleSerializer())
    ):
        self._jrdd = jrdd
    ...
    def mapPartitionsWithSplit(self, f, preservesPartitioning=False):
        ...
        """
        warnings.warn(
            "mapPartitionsWithSplit is deprecated; "
    ...
    def union(self, other):
        ...
        if self._jrdd_deserializer == other._jrdd_deserializer:
            ...
            rdd = RDD(
                self_copy._jrdd.union(other_copy._jrdd), self.ctx, self.ctx.serializer
            )
        if (
    ...
    def sortByKey(self, ascending=True, numPartitions=None, keyfunc=lambda x: x):
        ...
        def sortPartition(iterator):
            sort = ExternalSorter(memory * 0.9, serializer).sorted
            return iter(
                sort(iterator, key=lambda kv: keyfunc(kv[0]), reverse=(not ascending))
            )
        ...
        return self.partitionBy(numPartitions, rangePartitioner).mapPartitions(
            sortPartition, True
        )
    ...
    def cartesian(self, other):
        ...
        deserializer = CartesianDeserializer(
            self._jrdd_deserializer, other._jrdd_deserializer
        )
    ...
    def takeOrdered(self, num, key=None):
        ...
        return self.mapPartitions(lambda it: [heapq.nsmallest(num, it, key)]).reduce(
            merge
        )
        ...
        while len(items) < num and partsScanned < totalParts:
            ...
            if partsScanned > 0:
                ...
                if len(items) == 0:
                    ...
                    numPartsToTry = (
                        int(1.5 * num * partsScanned / len(items)) - partsScanned
                    )
    ...
    def saveAsTextFile(self, path, compressionCodecClass=None):
        ...
        if compressionCodecClass:
            ...
            keyed._jrdd.map(self.ctx._jvm.BytesToString()).saveAsTextFile(
                path, compressionCodec
            )
        else:
    ...
    def partitionBy(self, numPartitions, partitionFunc=portable_hash):
        ...
        limit = (
            _parse_memory(self.ctx._conf.get("spark.python.worker.memory", "512m")) / 2
        )
        ...
        with SCCallSiteSync(self.context) as css:
            ...
            jpartitioner = self.ctx._jvm.PythonPartitioner(
    ...
    def foldByKey(
        self, zeroValue, func, numPartitions=None, partitionFunc=portable_hash
    ):
        """
