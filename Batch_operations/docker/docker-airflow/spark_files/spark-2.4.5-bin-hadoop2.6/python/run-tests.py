...
def run_individual_python_test(target_dir, test_name, pyspark_python):
    ...
    if retcode != 0:
        ...
        try:
            ...
            decoded_lines = map(
                lambda line: line.decode("utf-8", "replace"), iter(per_test_output)
            )
            skipped_tests = list(
                filter(
                    lambda line: re.search(
                        r"test_.* \(pyspark\..*\) ... skipped ", line
                    ),
                    decoded_lines,
                )
            )
...
def get_default_python_executables():
    ...
    if "python2.7" not in python_execs:
        LOGGER.warning(
            "Not testing against `python2.7` because it could not be found; falling"
...
def parse_opts():
    parser = OptionParser(prog="run-tests")
    parser.add_option(
        "--python-executables",
        type="string",
    ...
    parser.add_option(
        "--verbose", action="store_true", help="Enable additional debug logging"
    )
...
def main():
    ...
    LOGGER.info(
        "Will test the following Python modules: %s", [x.name for x in modules_to_test]
    )
    ...
    for python_exec in python_execs:
        ...
        LOGGER.debug(
            "%s python_implementation is %s", python_exec, python_implementation
        )
        LOGGER.debug(
            "%s version is: %s",
            python_exec,
